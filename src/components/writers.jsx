...
const Writers = ({category ,id}) => {
  const [FilterWriters, setFilterWriters] = useState([])
  const [Items, setItems] = useState(8)

useEffect(() => {
  category ? setFilterWriters(writers.filter((writer)=> writer.writeAbout == category).filter((writer)=> writer.id != id)) : setFilterWriters(writers)
}, [category, id])


console.log(FilterWriters)
  useEffect(() => {
  ...
  return (
    <div className="writers flex  justify-center items-center flex-col  gap-2 mt-6">
      <h1 className='text-4xl sm:text-5xl font-extrabold text-gray-900 text-center mb-4 '>{category ? "Related" : "Top"} <span className='text-blue-600'>Writers</span></h1>
