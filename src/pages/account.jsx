...
const Account = () => {
  ...
  useEffect(() => {
    setAccount(writers.filter((writer) => writer.id == id));
  }, [id])
  
  // Calculate category after Account state updates
  const category = Account.length > 0 ? Account[0].writeAbout : undefined
          ...
          >
            <div className="mainContainer flex w-full  gap-2 justify-evenly items-center py-3">
              ...
              <div className="infoContainer flex flex-col justify-center items-start md:items-start gap-1">
                ...
                <span className="text-base text-blue-500 font-medium -mt-2">@{Account.username}</span>

                <div className="followers md:flex hidden w-full justify-between pr-4 items-center mb-2">
                  <span className="  flex flex-col justify-center items-center"><span className='text-blue-700 font-semibold text-lg'> {Account.noOfFollowing || "0"}</span><span className=' font-medium' >Following </span></span>
                  ...
                  <span className=" flex flex-col  justify-center items-center"><span className='text-blue-700 font-semibold text-lg'>{Account.noOfPosts || "0"}</span><span className=' font-medium'  >Posts </span></span>
                  <span className="  flex flex-col justify-center items-center"><span className='text-blue-700 font-semibold text-lg'> {Account.noOfLikes || "0"}</span><span className=' font-medium' >Likes </span></span>
                </div>
